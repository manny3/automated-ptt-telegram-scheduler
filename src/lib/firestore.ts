import { Firestore, DocumentData, QuerySnapshot, DocumentSnapshot } from '@google-cloud/firestore'
import { 
...
function documentToConfiguration(doc: DocumentSnapshot): ScrapingConfiguration | null {
  if (!doc.exists) {
    return null
  }
  ...
  return {
    ...
    lastExecuted: data.lastExecuted ? firestoreTimestampToDate(data.lastExecuted) : undefined,
 ...
 */
export async function getActiveConfigurations(): Promise<ScrapingConfiguration[]> {
  const db = getFirestore()
 ...
 */
export async function getConfigurationById(id: string): Promise<ScrapingConfiguration | null> {
  const db = getFirestore()
...
export async function updateConfigurationStatus(
  ...
  try {
    await db.collection(COLLECTIONS.CONFIGURATIONS).doc(id).update({
      lastExecuted: now,
 ...
 */
export async function getRecentExecutions(limit: number = 100): Promise<ExecutionResult[]> {
  const db = getFirestore()
...
export async function cleanupOldExecutions(): Promise<void> {
  ...
  try {
    // Get all configurations
    const configsSnapshot = await db.collection(COLLECTIONS.CONFIGURATIONS).get()
    ...
    for (const configDoc of configsSnapshot.docs) {
      ...
      if (executionsSnapshot.size > 1000) {
        const toDelete = executionsSnapshot.docs.slice(0, executionsSnapshot.size - 1000)
        ...
        await batch.commit()
        console.log(`Cleaned up ${toDelete.length} old executions for config ${configId}`)
      }
    }
  } catch (error) {
